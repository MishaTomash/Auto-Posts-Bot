...
const getChannelSettingsKeyboard = (ch, user) => {
    ...
    const canUseAI = user?.role === 'admin' || user?.subscription?.hasCustomPrompt === true || (plan !== 'free' && plan !== 'FREE');
    ...
    return [
        ...
        [{ text: '⬅️ Мої проєкти', callback_data: 'list_channels' }, { text: '🏠 Меню', callback_data: 'main_menu' }]
    ];
};

const getIntervalKeyboard = (ch) => {
    // Використовуємо універсальне отримання ID
